...
int main()
{
  ...
  fbputs("Hello CSEE 4840 World!", 4, 10); // print
  ...
  // clean screen
  fbputs("Press any key to clean screen", 13, 0); // row col
  for (;;) {
  ...
  for (;;) {
    libusb_interrupt_transfer(keyboard, endpoint_address, (unsigned char *) &packet, sizeof(packet), &transferred, 0);
    if (transferred == sizeof(packet)) {
        // modifiers crtl /  k0 k1
        sprintf(keystate, "%02x %02x %02x", packet.modifiers, packet.keycode[0], packet.keycode[1]);
        printf("%s\n", keystate);

        // 1. keystate to char "01 03 04"
        int m = get_acsii(keystate, 0, 1);
        int key1 = get_acsii(keystate, 3, 4);
        int key2 = get_acsii(keystate, 6, 7);
        printf("%d, %d, %d\n", m , key1, key2);
        ...
        fbputs(keystate, 6, 0); // row col
        char test[3] = " ";
        test[1] = key1;
        test[2] = key2;
        fbputs(test, TEXT_START_ROW + 1, 0); // row col  several text matain a buff
